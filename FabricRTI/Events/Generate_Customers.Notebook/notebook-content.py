...
# 
# This notebook generates realistic synthetic customer profiles for the **Real-Time Intelligence Fraud Detection demo** on Microsoft Fabric, and ingests them directly into the **Customers** table of an Eventhouse KQL database.
# 
# **What it produces:**
...
# - **Direct ingestion** — Rows are pushed into the Eventhouse `Customers` table via the Kusto ingest endpoint
# 
# **Customers are distributed across cities in the United States, Canada, Brazil, and Costa Rica.**
# 
# **Run this notebook before** `Generate_Credit_Card_Transactions` — that notebook reads from the same `Customers` table.
...
%pip install faker azure-kusto-data azure-kusto-ingest --quiet
...
# ## 2. Configuration
# 
# Set the number of users to generate, your Entra ID tenant domain, and the Eventhouse KQL database connection.
...
CUSTOMERS_TABLE = "Customers"                                            # Target table
...
print(f"Target Table:      {CUSTOMERS_TABLE}")
...
def generate_user_profiles(n_users):
    ...
    for i in range(n_users):
        ...
        profile = {
            ...
            "credit_limit": float(random.choice([3000, 5000, 7500, 10000, 15000, 20000, 30000, 50000])),
...
# ## 5. Ingest Customers into the Eventhouse
# 
# Push the generated profiles directly into the Eventhouse `Customers` table via the Kusto ingest endpoint. The dataframe columns are ordered to match the table schema defined in `DatabaseSchema.kql`, so no ingestion mapping is required.
# 
# > **Note:** Ingestion is queued and typically becomes queryable within 1–5 minutes.
...
# ── Connect to the Eventhouse ingest endpoint ─────────────
INGESTION_URI = EVENTHOUSE_URI.replace("https://", "https://ingest-")
...
kcsb = KustoConnectionStringBuilder.with_aad_application_token_authentication(
    INGESTION_URI, access_token
)
ingest_client = QueuedIngestClient(kcsb)

# ── Order columns to match the Customers table schema ─────
ordered_cols = [
    "user_id", "first_name", "last_name", "display_name", "user_principal_name", "email",
    "age", "gender", "street_address", "home_city", "home_state", "postal_code",
    "country_code", "country", "home_lat", "home_lon", "credit_limit", "account_created",
]
ordered_df = users_df[ordered_cols].copy()
# Ensure account_created serializes as an ISO date string
ordered_df["account_created"] = pd.to_datetime(ordered_df["account_created"]).dt.strftime("%Y-%m-%d")
...
ingest_client.ingest_from_dataframe(ordered_df, ingestion_properties=ingestion_props)

print(f"✅ Queued ingestion of {len(ordered_df)} customers into {KQL_DATABASE}.{CUSTOMERS_TABLE}")
print(f"   Ingest endpoint: {INGESTION_URI}")
print(f"\n💡 Wait 1–5 minutes, then verify in your KQL queryset:")
print(f"   {CUSTOMERS_TABLE} | count")
print(f"\n   Next: run Generate_Credit_Card_Transactions — it queries this table for user profiles.")
